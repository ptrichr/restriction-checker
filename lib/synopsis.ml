let read_string str = 
  ...
  List.fold_left (fun a item -> Utils.get_synopsis item a) {modules = []; definitions = []} tree

let read_file src = 
  let inchan = open_in src in
  let parsetree = Parse.implementation (Lexing.from_channel inchan) in
  List.fold_left (fun a item -> Utils.get_synopsis item a) {modules = []; definitions = []} parsetree
